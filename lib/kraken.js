var Memcached = require('mc').Client;
...
var Kraken = function(host, port) {
  this._host = host;
  this._port = port;
  this._memcached = new Memcached(this._host + ":" + this._port);
};

Kraken.prototype.connect = function(callback) {
  this._memcached.connect(callback);
};

Kraken.prototype.disconnect = function() {
  this._memcached.disconnect();
};
...
Kraken.prototype.subscribe = function(topics, callback) {
  if (topics.length > 0) {
    this._memcached.set(
        "subscribe", topics.join(" "), {flags: 0, exptime: 0}, callback);
  }
};
...
Kraken.prototype.unsubscribe = function(topics, callback) {
  if (topics.length > 0) {
    this._memcached.set(
        "unsubscribe", topics.join(" "), {flags: 0, exptime: 0}, callback);
  }
};
...
Kraken.prototype.publish = function(topic_message_pairs, callback) {
  if (topic_message_pairs.length > 0) {
    ...
    this._memcached.set(
        "publish", buffer.join(""), {flags: 0, exptime: 0}, callback);
  }
};
...
Kraken.prototype.receiveMessages = function(callback) {
  this._memcached.get("messages", function(err, result) {
    if (err) {
      ...
      if (result !== undefined) {
        var buffer = new Buffer(result.messages);
